...
export class RoleFormComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.data) {
      const _permissions = this.data?.permissions?.map((permission: any) => permission?.name)
      const _payload = {
        name: this.data?.name,
  ...
  onSubmit(): void {
    ...
    if (this.data) {
      this._roleService.updateRole(this?.data?.name, payload).subscribe(_ => {
      ...
      this._roleService.createRole(payload).subscribe(_ => {
        this.snackBar.open('Success! Role created.', 'Close', {
          duration: 4000
        })
        this.dialogRef.close(true);

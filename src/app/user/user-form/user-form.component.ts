...
export class UserFormComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    if (this.userData) {
      this._userService.updateUser(this.userId, _formData).subscribe(_ => {
      ...
      this._userService.createUser(_formData).subscribe(_ => {
  ...
  // Dependencies
  getResources(): void {
    this._userService.getResources().subscribe(res => {
  ...
  addPasswordControl(): void {
    ...
    this.userForm.addControl('password', this.fb.control('', [Validators.required, Validators.minLength(10)]));
